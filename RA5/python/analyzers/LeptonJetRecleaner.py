...
class LeptonJetRecleaner:
    ...
    def __init__(self,name,label,looseLeptonSel,cleaningLeptonSel,FOLeptonSel,tightLeptonSel,cleanJet,selectJet,cleanTau,looseTau,tightTau,cleanJetsWithTaus,doVetoZ,doVetoLMf,doVetoLMt,jetPt,bJetPt,coneptdef,storeJetVariables=False,cleanTausWithLoose=False,jetVarToStoreDict={}):
        self.name = name
        ...
        self.jetVarToStoreDict = jetVarToStoreDict
    ...
    def fillCollWithVeto(self,ret,refcollection,leps,lab,labext,selection,lepsforveto,doVetoZ,doVetoLM,sortby,ht=-1,pad_zeros_up_to=20):
        ...
        ret['LepGood_is'+labext+'Out'] = [1 if i in ret['i'+lab] else 0 for i in xrange(len(refcollection))]
        ...
        ret['LepGood_is'+labext+'VetoOut'] = [1 if i in ret['i'+lab+'V'] else 0 for i in xrange(len(refcollection))]
    ...
    def recleanJets(self,jetcollcleaned,jetcolldiscarded,lepcoll,postfix,ret,jetret,discjetret):
        ...
        if self.storeJetVariables:
            ...
            if postfix==self.label:
                for jfloat in self.jetVarToStoreDict:
                    jetret[jfloat] = []
                    discjetret[jfloat] = []
                for idx in ret["iJSel"+postfix]:
                    jet = jetcollcleaned[idx] if idx >= 0 else jetcolldiscarded[-1-idx]
                    for jfloat,func in self.jetVarToStoreDict.iteritems():
                        jetret[jfloat].append( func(jet) )
                for idx in ret["iDiscJSel"+postfix]:
                    jet = jetcollcleaned[idx] if idx >= 0 else jetcolldiscarded[-1-idx]
                    for jfloat,func in self.jetVarToStoreDict.iteritems():
                        discjetret[jfloat].append( func(jet) )
         # 5. compute the sums
        ...
        for x in lepcoll: 
            ...
            tempVec.SetPtEtaPhiM(x.pt(),x.eta(),x.phi(),x.mass())
            mhtBJetPtvec = mhtBJetPtvec - tempVec
        for x in lepcoll: 
            tempVec = ROOT.TLorentzVector()
            tempVec.SetPtEtaPhiM(x.pt(),x.eta(),x.phi(),x.mass())
            mhtJetPtvec = mhtJetPtvec - tempVec
        for j in jetcollcleaned+jetcolldiscarded:
            ...
            if j.pt() > float(self.bJetPt):
                ...
                j.btagCSV = j.btag('pfCombinedInclusiveSecondaryVertexV2BJetTags')
                ...
                jVec.SetPtEtaPhiM(j.pt(),j.eta(),j.phi(),j.mass())
                mhtBJetPtvec = mhtBJetPtvec - jVec
    ...
    def __call__(self,event):
        ...
        leps = [l for l in event.selectedLeptons]
        ...
        tausc = [t for t in event.selectedTaus]
        #tausd = [t for t in Collection(event,"TauOther","nTauOther")] 
        tausd = [t for t in event.otherTaus] 
        jetsc={}
        jetsd={}
        for var in self.systsJEC:
            ...
            jetsc[var] = [j for j in getattr(event,"cleanJets"+self.systsJEC[var])]
            #jetsd[var] = [j for j in Collection(event,"DiscJet"+self.systsJEC[_var],"nDiscJet"+self.systsJEC[_var])]
            jetsd[var] = [j for j in getattr(event,"discardedJets"+self.systsJEC[var])]
      ...
      for l1 in lepst:
        ...
        for l2 in lepsl:
            ...
            if l1.pdgId() == -l2.pdgId():
               ...
               l1Vec.SetPtEtaPhiM(l1.pt(),l1.eta(),l1.phi(),l1.mass())
               l2Vec = ROOT.TLorentzVector()
               l2Vec.SetPtEtaPhiM(l2.pt(),l2.eta(),l2.phi(),l2.mass())
               mz = (l1Vec + l2Vec).M()
               diff = abs(mz-91)
